...
public class EcranFin implements Screen {
    ...
    public EcranFin(GalaxyFighter jeu){
        parent = jeu;
        ...
        fin = new Label("Bravo a toi " + parent.joueur.getPseudo() + " !", new Label.LabelStyle(new BitmapFont(), Color.WHITE));
        ...
        table.top();
        table.add(fin).padTop(10).padLeft(15).fillX();
        table.row();
        table.add(this.score).padTop(90);
        table.row();
        table.add(retourMenu).padTop(200);

        winSound = Gdx.audio.newSound(Gdx.files.internal("musics_n_sounds/sounds/win.mp3"));
        if (parent.musiqueB)
            winSound.play();
